...
class BaseApi:
    ...
    def _request(
        self, method: str, url: str, json=None, files=None, data=None, params: dict = None, headers: dict = None, **_
    ):
        request = requests.Request(
            method=method, url=url, json=json, files=files, data=data, params=params, headers=headers, auth=self._auth
        ).prepare()
        log_request(logger=loguru.logger, request=request)
        with requests.Session() as session:
            response = session.send(request=request, verify=False)
